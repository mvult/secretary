...
async def analyze_transcript(
    ...
    speaker_mappings: List[Dict] = None,
) -> Dict[str, Any]:
            ...
            if speaker_mappings:
                ...
                for mapping in speaker_mappings:
                    user_name = user_map.get(
                        mapping.get("user_id"), f"User {mapping.get('user_id')}"
                    )
...
{transcript}"""
            ...
            prompt = (
                "Provide a comprehensive summary of this transcript in approximately 200 words. Include key topics discussed, main points, decisions made, any important outcomes, and any pending questions or unresolved issues:\n\n"
                + transcript
            )
        else:
            return {
        ...
        if analysis_type == "todos":
            try:
                ...
                if todos and recording_id:
                    ...
                    logging.info(
                        f"Saved {saved_count}/{len(todos)} TODOs to database for recording {recording_id}"
                    )
