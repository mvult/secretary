...
class RecordingService:
    ...
    async def create_recording(
        name: str, local_audio_path: str = None, duration: int = None, notes: str = None
    ) -> Optional[Recording]:
        """Create a new recording entry"""
        try:
            recording = await Recording.create(
                name=name, local_audio=local_audio_path, duration=duration, notes=notes
            )
            return recording
        ...
        try:
            ...
            for recording in recordings:
                ...
                setattr(
                    recording,
                    "transcript_status",
                    "Ready" if has_transcript else "Missing",
                )
...
class UserService:
    ...
    async def create_user(
        first_name: str, last_name: str, role: str = None
    ) -> Optional[User]:
        """Create a new user"""
        try:
    ...
    async def save_speaker_mappings(recording_id: int, mappings: List[Dict]) -> bool:
        ...
        try:
            ...
            await db.execute_query(
                "DELETE FROM speaker_to_user WHERE recording_id = $1", [recording_id]
            )
...
class AnalysisService:
    ...
    @staticmethod
    async def get_output_status(recording) -> Dict[str, bool]:
        """Get structured output availability for a recording."""
        try:
            speaker_mappings = await SpeakerService.get_speaker_mappings(recording.id)
            has_speakers = len(speaker_mappings) > 0
            has_summary = bool(recording.summary)
            ...
            return {
                "transcript": bool(recording.transcript),
                "speakers": has_speakers,
                "todos": has_todos,
                "summary": has_summary,
            }
        except Exception as e:
            logging.error(f"Error getting output status: {e}")
            return {
                "transcript": bool(getattr(recording, "transcript", None)),
                "speakers": False,
                "todos": False,
                "summary": bool(getattr(recording, "summary", None)),
            }
...
class TodoService:
    ...
    async def create_todo(
        ...
        status: str = "todo",
        user_id: int = None,
        created_at_recording_id: int = None,
        updated_at_recording_id: int = None,
    ) -> bool:
        """Create a new TODO"""
        try:
            from db.models import Todo
